    ...
    def __init__(
        ...
        **kwargs,
        ...
        if len(samples_root) > 0:
            wnid_exists = [
                entry.name for entry in os.scandir(samples_root) if entry.is_dir()
            ]

            # check fot empty dirs
            wnid_exists = [
                x
                for x in wnid_exists
                if len(os.listdir(os.path.join(samples_root, x))) > 0
            ]
            # import pdb; pdb.set_trace()

            wnid_to_idx = {wnid: self.wnid_to_idx[wnid] for wnid in wnid_exists}

            samples_done = self.make_dataset(
                samples_root, wnid_to_idx, extensions=IMG_EXTENSIONS
            )
    ...
    **kwargs,
    ...
    if transform == "diffusion":
        transform = transforms.Compose(
            [partial(center_crop_arr, image_size=image_size), transforms.ToTensor()]
        )
    ...
    dset = ImageNet(
        ...
        samples_root=(
            samples_root if not overwrite and len(os.listdir(samples_root)) > 0 else ""
        ),
        meta_root=meta_root,
    )
    return dset
...
def get_imagenet_loader(
    dset, *, batch_size, num_workers, shuffle, drop_last, pin_memory, **kwargs
):
