...
from sampler import ConjugateV2
...
def create_degradation(args):
    ...
    if name[:2] == "sr":
        ...
        deg = Quantization(int(name[5:]), norm=True if "lsun" not in args.dataset else False)
    elif name[:5] == "phase":
        oversample=2.0
        deg = PhaseRetrievalOperator(img_shape[0], img_shape[1], oversample=oversample, device=args.device)
    elif name[:6] == "deblur":
        sigma = 3
        window = 61
...
def main():
    args = create_args()
    conj_dir = os.path.join(str(args.out_dir), str(args.dataset), 'sample_' + str(args.n_sample_steps), 'conj', 'lamb_' + str(args.lamb), 'w_' + str(args.w), 'st_' + str(args.skip_t))
    # print(conj_dir)
    ...
    sampler = ConjugateV2(model, deg, w=args.w, skip_t=args.skip_t, B=args.lamb, integral_start=0)
    ...
    for batch_idx, (unormed_image, _) in enumerate(dl):
        ...
        normed_rec_img_conj = sampler.sample_conj(y, img_dim, n_steps=args.n_sample_steps)
        # normed_rec_img_conj = sampler.sample(unormed_image.shape, sample_steps=args.n_sample_steps)
        recon = deg.H_pinv(y).reshape(*unormed_image.shape)
        if "lsun" in args.dataset:
            unormed_rec_img_conj = normed_rec_img_conj.clamp(0, 1)
            recon = recon.clamp(0, 1)
        else:
            unormed_rec_img_conj = unnormalize(normed_rec_img_conj).clamp(0, 1)
            recon = unnormalize(recon).clamp(0, 1)

        with torch.no_grad():
            if "lsun" in args.dataset:
                lpips_val += loss_fn_alex(normalize(normed_rec_img_conj), normalize(normed_image)).sum()
            else:
                lpips_val += loss_fn_alex(normed_rec_img_conj, normed_image).sum()
            psnr_val += psnr(unormed_rec_img_conj, unormed_image).sum()
            ms_ssim_val += ms_ssim(unormed_rec_img_conj, unormed_image, size_average=False).sum()
        ...
        pathlib.Path(os.path.join(args.out_dir, str(args.dataset), "degraded")).mkdir(parents=True, exist_ok=True)
        pathlib.Path(conj_dir).mkdir(parents=True, exist_ok=True)
        ...
        for datum_idx in range(unormed_image.shape[0]):
            ...
            # torchvision.utils.save_image(unormed_rec_img_pgdm.cpu()[datum_idx], os.path.join(config.out_dir, "pgdm", f"pgdm_{batch_idx}_{datum_idx}.png"))
            torchvision.utils.save_image(unormed_rec_img_conj.cpu()[datum_idx], os.path.join(conj_dir, f"conj_{batch_idx}_{datum_idx}.png"))
            # if not os.path.isfile(os.path.join(args.out_dir, str(args.dataset), "degraded_pinv", f"degrad_pinv_{batch_idx}_{datum_idx}.png")):
            if args.degradation == "deblur":
                if "lsun" in args.dataset:
                    torchvision.utils.save_image(y.cpu()[datum_idx].reshape(*unormed_image.shape[1:]), os.path.join(args.out_dir, str(args.dataset), "degraded", f"degrad_{batch_idx}_{datum_idx}.png"))
                else:
                    torchvision.utils.save_image((y.cpu()[datum_idx].reshape(*unormed_image.shape[1:]) + 1) / 2, os.path.join(args.out_dir, str(args.dataset), "degraded", f"degrad_{batch_idx}_{datum_idx}.png"))
            elif args.degradation == "sr4":
                c, h, w = unormed_image.shape[1:]
                h = h // int(args.degradation[2:])
                w = w // int(args.degradation[2:])
                if "lsun" in args.dataset:
                    torchvision.utils.save_image(y.cpu()[datum_idx].reshape(c, h, w), os.path.join(args.out_dir, str(args.dataset), "degraded", f"degrad_{batch_idx}_{datum_idx}.png"))
                else:
                    torchvision.utils.save_image((y.cpu()[datum_idx].reshape(c, h, w) + 1) / 2, os.path.join(args.out_dir, str(args.dataset), "degraded", f"degrad_{batch_idx}_{datum_idx}.png"))
    ...
    with open(os.path.join(conj_dir, 'lpips.txt'), 'w') as f:
        f.write(f"lpips: {(lpips_val / num_sample).item()}")
    with open(os.path.join(conj_dir, 'psnr.txt'), 'w') as f:
        f.write(f"lpips: {(psnr_val / num_sample).item()}")
    with open(os.path.join(conj_dir, 'ms_ssim.txt'), 'w') as f:
