...
from compressai.zoo import mbt2018_mean, bmshj2018_factorized
import yaml
...
class H_functions:
    ...
    def H(self, vec):
        ...
        returned = self.U(singulars * temp[:, : singulars.shape[0]])
        return returned
...
class NaiveInpainting(H_functions):
    def __init__(self, channels, img_dim, mask_file, device, norm=True):
        self.channels = channels
        self.img_dim = img_dim
        self.device = device
        self.mask = torch.from_numpy(np.load(mask_file)['m']).float().to(device)
        # self.norm = norm
...
class JPEG(H_functions):
    def __init__(self, qf, shape, norm=True):
        self.qf = qf
        self.shape = shape
        self.norm = norm
    ...
    def H(self, image):
        ...
        """
        image = image.reshape(-1, *self.shape)
        return jpeg_decode(jpeg_encode(image, self.qf, self.norm), self.qf, self.norm)
...
class NeuralCompression(H_functions):
    def __init__(self, quality, shape, device, norm=True):
        self.device = device
        self.model = mbt2018_mean(quality=quality, metric='mse', pretrained=True).to(device)
        self.model.eval()
        self.shape = shape
        self.norm = norm
    ...
    def H(self, image):
        ...
        if self.norm:
            image = (image + 1) / 2
        y = self.model.g_a(image)
        z = self.model.h_a(y)
        z_hat, z_likelihoods = self.model.entropy_bottleneck(z)
        gaussian_params = self.model.h_s(z_hat)
        scales_hat, means_hat = gaussian_params.chunk(2, 1)
        y_hat, y_likelihoods = self.model.gaussian_conditional(y, scales_hat, means=means_hat)
        return y_hat

    def H_pinv(self, x):
        if self.norm:
            return self.model.g_s(x).reshape(x.shape[0], -1) * 2 - 1
        else:
            return self.model.g_s(x).reshape(x.shape[0], -1)

class NeuralCompressionFactorized(H_functions):
    def __init__(self, quality, shape, device, norm=True):
        self.device = device
        self.model = bmshj2018_factorized(quality=quality, metric='mse', pretrained=True).to(device)
        # mbt2018_mean, bmshj2018_factorized
        self.model.eval()
        self.shape = shape
        self.norm = norm
...
class Quantization(H_functions):
    def __init__(self, n_bits, norm=True):
        self.n_bits = n_bits
        self.norm = norm
...
from deg_utils.fft_utils import fft2_m, ifft2_m   

#@register_operator(name='phase_retrieval')
class PhaseRetrievalOperator(NonLinearOperator):
    def __init__(self, channel, image_dim, oversample, device):
    ...
    def forward(self, data, **kwargs):
        data = data.reshape(-1, self.channels, self.img_dim, self.img_dim)
    ...
    def H_pinv(self, x):
        ...
        x = x.reshape(-1, self.channels * self.img_dim ** 2)
        return x
